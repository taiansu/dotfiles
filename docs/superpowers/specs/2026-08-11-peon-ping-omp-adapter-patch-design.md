...
3. 監聽已通過 `tool_call` middleware 的 `tool_execution_start`；只有 main UI 執行 `ask` 時才送出 `Notification / elicitation_dialog`，避免被 block 的 `ask` 產生假通知。
...
6. 觸發已開始執行的 `ask`，收到一次 `input.required` payload；未進入 execution stage 的 blocked `ask` 不通知。
